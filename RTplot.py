...
def update_plot(frame):
    ...
    while ser.in_waiting:
        try:
            ...
            if ',' in line:
                parts = line.split(',')
                # Expecting 5 values: x, x_dot, theta, theta_dot, u
                if len(parts) == 5:
                    timestamps.append(time.time() - start_time)
                    linear_positions.append(float(parts[0]))
                    linear_velocities.append(float(parts[1]))
                    angles.append(float(parts[2]))
                    angular_velocities.append(float(parts[3]))
                    control_inputs.append(float(parts[4]))
        except (ValueError, UnicodeDecodeError):
            pass
    ...
    ax1.set_title('Real-Time 4D State Controller Performance', fontweight='bold')
    ax1.axhline(0, color='black', linestyle='--', linewidth=1)
    ax1.grid(True, linestyle=':', alpha=0.7)
    ...
    # 3. Control Input
    ax3.plot(timestamps, control_inputs, color='red', linewidth=2, label='Accel Input (rad/s²)')
    ax3.set_xlabel('Time (s)', fontweight='bold')
    ax3.set_ylabel('Control Input\n(rad/s²)', fontweight='bold')
    ax3.axhline(0, color='black', linestyle='--', linewidth=1)
    ax3.grid(True, linestyle=':', alpha=0.7)
